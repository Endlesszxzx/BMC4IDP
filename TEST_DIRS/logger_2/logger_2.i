...
inline void start(uint32_t _startTime,uint16_t _interval)
{
  ...
  interval = _interval;
  systemState = 1;
  ((void) sizeof ((numberOfRecords==0) ? 1 : 0), __extension__ ({ if (numberOfRecords==0) ; else __assert_fail ("numberOfRecords==0", "logger_2.c", 57, __extension__ __PRETTY_FUNCTION__); }));
}
