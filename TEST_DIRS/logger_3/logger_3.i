...
{
  if (numberOfRecords+1 >= 64) systemState = 0;
  periodCounter = 0;
  tickCounter = 0;
  numberOfRecords = 0;
...
void task_measure(void)
{
  __CPROVER_assume(systemState==1);
  tickCounter++;
  if (tickCounter == period) {
    ...
    numberOfRecords++;
    ((void) sizeof ((numberOfRecords==pos+1) ? 1 : 0), __extension__ ({ if (numberOfRecords==pos+1) ; else __assert_fail ("numberOfRecords==pos+1", "po_logger2.c", 72, __extension__ __PRETTY_FUNCTION__); }));
  }
}
...
void task_ext_power(void) {
  ...
  if(power && systemState==2) systemState = 0;
  if(!power && systemState==0) systemState = 2;
}
...
void task_communicate(void)
{
  __CPROVER_ASYNC_1:
  task_measure();
  __CPROVER_assume(systemState!=2);
  uint8_t len = receive_cmd(msgBuffer);
  uint8_t cmd = msgBuffer[0];
  uint8_t decodingStatus = 0;
  if(cmd==2 || cmd==3 || cmd==4 || cmd==1) decodingStatus = 1; if (decodingStatus != 1) {
    msgBuffer[0] = 0;
