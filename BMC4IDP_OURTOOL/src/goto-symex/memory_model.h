...
protected:
  ...
  unsigned var_cnt;
  virtual symbol_exprt nondet_bool_symbol(const std::string &prefix);
  ...
  constrains_t rf_relations;
    virtual /// For each read `r` from every address we collect the choice symbols `S`
