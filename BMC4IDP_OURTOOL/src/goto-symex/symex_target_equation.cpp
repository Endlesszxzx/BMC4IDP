  ...
  const sourcet &source)
{
  ...
  SSA_stept &SSA_step=SSA_steps.back();
#ifndef NDEBUG
  static std::set<std::string> marked = {
    "rf-begin","rf-end","ws-begin","ws-end","fr-begin","fr-end"
  };
  if(marked.find(msg)!=marked.end()){
    SSA_step.marked = true;
  }
#endif
  ...
  decision_proceduret &decision_procedure)
{
  ...
  for(auto &step : SSA_steps)
  {
    if(step.is_constraint() && !step.ignore && !step.converted)
    {
      ...
      if(step.marked){
        decision_procedure.marked_set_to_true(step.cond_expr);
      }else{
        decision_procedure.set_to_true(step.cond_expr);
      }
      step.converted = true;
