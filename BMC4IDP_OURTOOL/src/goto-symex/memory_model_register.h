...
using memory_model_maker=std::function<std::unique_ptr<memory_model_baset>(const namespacet &ns)>;
...
#define register_memory_model(argus,class_name) static_assert(std::is_base_of<memory_model_baset,class_name>::value||std::is_same<class_name,memory_model_baset>::value,"error class name");memory_model_handlers.insert({#argus,[](const namespacet &ns)->std::unique_ptr<memory_model_baset>{return util_make_unique<class_name>(ns);}})
