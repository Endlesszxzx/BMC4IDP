...
extern std::set<exprt> marked_cons;
...
#ifndef NDEBUG
void memory_model_sc_expand::print_event(const event_it &e) {
    std::string  type = "normal event";
    if(e->is_shared_read()){
        type = "r";
    } else if (e->is_shared_write()){
        type = "w";
    } else if (e->is_spawn()){
        type = "spawn";
    }
    std::cout<<"function:"<<e->source.function_id<<" event_type:"<<type<<" address:"<<e->ssa_lhs.get_object_name()<<std::endl;
}
#endif
#endif
