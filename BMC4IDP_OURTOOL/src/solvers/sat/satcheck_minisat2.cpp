...
propt::resultt satcheck_minisat2_baset<T>::do_prop_solve()
{
    ...
    try
  {
    ...
    log.statistics()<<"decisions: "<<solver->decisions<<messaget::eom;
    log.statistics()<<"propagations: "<<solver->propagations<<messaget::eom;
    log.statistics()<<"conflicts: "<<solver->conflicts<<messaget::eom;
    if(old_handler != SIG_ERR)
    {
      alarm(0);
